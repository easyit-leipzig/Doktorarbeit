...
SET @revision_code := 'RKB-REV-K3.4.5-P1-V1';
...
(
    SELECT source_id
    FROM sources
    WHERE source_key='rudin_functional_analysis_1991'
    LIMIT 1
);
...
(
    SELECT source_id
    FROM sources
    WHERE source_key='dummit_foote_abstract_algebra_2004'
    LIMIT 1
);
...
(
    SELECT source_id
    FROM sources
    WHERE source_key='conway_functional_analysis_1990'
    LIMIT 1
);
...
(
    CASE
        ...
        WHEN @missing_sources > 0
            THEN 'Mindestens eine erforderliche Bestandsquelle fehlt: Rudin [11], Dummit/Foote [31] oder Conway [35].'
        ELSE NULL
    END
);
...
VALUES
('last_repository_revision', 'RKB-REV-K3.4.5-P1-V1')
ON DUPLICATE KEY UPDATE
counter_value='RKB-REV-K3.4.5-P1-V1';
...
SELECT
    ...
    summary
FROM repository_revisions
WHERE revision_code='RKB-REV-K3.4.5-P1-V1';
...
WHERE su.section_id=@section_id
  AND s.source_key IN
  (
      'rudin_functional_analysis_1991',
      'dummit_foote_abstract_algebra_2004',
      'conway_functional_analysis_1990'
  )
