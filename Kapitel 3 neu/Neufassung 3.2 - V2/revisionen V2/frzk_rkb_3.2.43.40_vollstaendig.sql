...
/* Repository-Update 3.2.43.40
Definitionen: 3.2.879-3.2.883
Sätze: 3.2.321-3.2.324
Gleichungen: (3.3732)-(3.3733)
Literatur: [152]
*/
...
VALUES
('RKB-NEU-K3.2.43.40-V1',NOW(),'subsection','3.2.43.40',
'3.2.43.40-v1-vollstaendig',
'Übergang zu Kapitel 3.3 und Abschluss Kapitel 3.2.',
'Olaf Thiele / ChatGPT')
...
SELECT 152,'transition_ch3_152','book',
'Übergang zu Kapitel 3.3 – Axiomatische Grundlagen des FRZK',
'de','pending'
WHERE NOT EXISTS (SELECT 1 FROM sources WHERE citation_number=152);

INSERT INTO section_change_log
(revision_id,section_id,change_type,object_type,object_reference,change_summary)
SELECT (SELECT MAX(revision_id) FROM repository_revisions),
section_id,'updated','subsection','3.2.43.40',
'Übergangskapitel vollständig ergänzt.'
...
VALUES
('last_completed_section','3.2.43.40'),
('current_section','3.3.1'),
('last_definition_number','3.2.883'),
('next_definition_number','3.3.1'),
('last_theorem_number','3.2.324'),
('next_theorem_number','3.3.1'),
('last_equation_number','3.3733'),
('next_equation_number','3.4001'),
('last_citation_number','152'),
('next_citation_number','153')
ON DUPLICATE KEY UPDATE counter_value=VALUES(counter_value);

COMMIT;

-- Definitionen:
-- 3.2.879 Axiomatischer Ausgangszustand
-- 3.2.880 Primäre Axiomenmenge
-- 3.2.881 Axiomatische Folgerung
-- 3.2.882 Geschlossene Axiomenbasis
-- 3.2.883 Übergangsabbildung

-- Sätze:
-- 3.2.321 Unabhängigkeit der Primäraxiome
-- 3.2.322 Hierarchie der Aussagen
-- 3.2.323 Geschlossenheit des FRZK
-- 3.2.324 Methodische Trennung
